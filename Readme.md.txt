...
2. `custom.review_count` (Integer) — Total verified buyer reviews count.
3. `custom.bundle_products` (List of products) — Component products for pre-packaged combo SKUs.
...
- Where AI Failed & Required Correction:
  AI initially hallucinated deprecated/non-standard Liquid tags (like `product.images[0].src` instead of modern `image_url` / `image_tag`), defaulted to static text rather than merchant-configurable settings, and omitted card height constraints causing long titles to break grid rows.
